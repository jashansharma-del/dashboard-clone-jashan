...
app.post("/api/ai/respond", async (req, res) => {
  ...
  const prompt = [
    ...
    "- assistantText: string (markdown version of insights)",
    "- chart?: { type: pie|bar|line, title?: string, series: [{label:string,value:number,forecast?:boolean}] }",
    "- commands?: [{ type: 'auto-layout', layout: 'circular'|'grid'|'horizontal' }, { type: 'add-node', nodeType: 'summary-node', data: { title: string, content: string } }]",
    "",
    "Guidelines:",
    "1. If user asks for a forecast, include extra points in 'series' with 'forecast: true'.",
    "2. If user asks to organize the canvas, use 'auto-layout' command.",
    "3. If board summary is requested, use 'add-node' command with 'summary-node'.",
    "",
